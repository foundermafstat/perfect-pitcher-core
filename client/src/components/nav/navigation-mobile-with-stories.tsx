...
export function NavigationMobileWithStories({ navItems, recentStories }: NavigationMobileProps) {
  const segment = useSelectedLayoutSegment()
  const [isOpen, setIsOpen] = React.useState<boolean>(false)
  const { locale } = useTranslations()
  const items = navItems || getNavigationItems(locale)
            ...
            <>
              ...
              {recentStories.map((story) => (
                <MobileLink
                  ...
                  setIsOpen={setIsOpen}
                >
                  <span className="text-base font-normal">{story.title}</span>
                </MobileLink>
              ))}
            </>
          )}
